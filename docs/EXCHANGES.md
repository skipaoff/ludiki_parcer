...
**Торговля через API** открыта с 31.03.2026 для всех пользователей с пройденной KYC. Нужен ключ с правом на фьючерсы.

**Комиссии API MEXC считаются отдельно от веба и приложения и уже дважды росли** [док, проверено 25.09.2026]:

| С какой даты | Мейкер | Тейкер |
|---|---|---|
| 31.03.2026 (запуск) | 0,01% | 0,05% |
| 01.05.2026 | 0,04% | 0,06% |
| 01.06.2026 — действует сейчас | 0,06% | **0,08%** |
...
| то же | `takerFeeRate` и `makerFeeRate` — публичные ставки по контракту; бывают нулевыми (зона `mc-trade-zone-0fees`). Ставки для API объявляются отдельно, поэтому до проверки ключей лента считает по 0,08% тейкер | [проверено 25.09.2026] |
...
| `wss://fstream.asterdex.com/stream`, `{"method":"SUBSCRIBE","params":[...]}` | один адрес для всех потоков, до 200 потоков на соединение, 10 управляющих сообщений в секунду, соединение живёт 24 часа. **`!bookTicker` (все символы) — в реальном времени**: за 15 с на BTC, ETH, SOL, DOGE, ASTER пришло ровно столько же обновлений, сколько в отдельных `<symbol>@bookTicker` (у Binance этот поток раз в 5 с). **Но терминал его не использует [проверено 15.09.2026]:** на всех символах он шёл со скоростью 6 600 сообщений в секунду — треть ядра процессора и задержки цикла событий до 300 мс. Радар Aster — опрос `GET /fapi/v1/ticker/bookTicker` (все символы) раз в секунду. `<symbol>@depth20@100ms` — формат Binance (`depthUpdate`, `b`/`a`), `!markPrice@arr@1s` |
...
### Ключи: API-кошелёк, а не API-ключ [док 25.09.2026]

- Терминал работает по V3 (подпись API-кошельком). Создание ключей V1 «ключ + секрет» биржа прекратила 25.03.2026, старые продолжают работать.
- Права агента задаются при создании: торговля перпетуалами включается, вывод — нет; при включённом выводе биржа требует список IP.
- **С 01.09.2026 приватные запросы V3 работают только после депозита основного кошелька**, иначе ответ `-5050 This function can only be used after deposit`.
- Как получить — `docs/KEYS.md`.
...
## Фандинг (все биржи, проверено 15.09.2026)

Ставки опрашиваются раз в минуту (`app/market/funding.py`), интервалы и расписания — раз в 30 минут. Знак везде один: **положительная ставка — лонги платят шортам**. Для пары «лонг на A, шорт на B» за расчёт лонг получает −ставку A, шорт получает +ставку B. Ставка старше 15 минут считается неизвестной, а не нулевой.

| Биржа | Ставка и время расчёта | Интервал |
|---|---|---|
| Binance | `GET /fapi/v1/premiumIndex`: `lastFundingRate` (доля), `nextFundingTime` | `GET /fapi/v1/fundingInfo` — `fundingIntervalHours`, только для контрактов с нестандартным интервалом (782 шт.: 4 ч — 465, 8 ч — 312, 1 ч — 5); остальные 8 ч |
| Aster | то же на `fapi.asterdex.com` | `fundingInfo`: 4 ч — 350, 8 ч — 303, 1 ч — 89, 2 ч — 3 |
