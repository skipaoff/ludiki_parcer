...
Ссылку вместе с токеном сессии терминал кладёт в файл, доступный только его пользователю:

```bash
sudo cat /opt/ludik/ludik-data/session-url.txt
```

Её и открыть в браузере на своей машине. Токен меняется при каждом перезапуске: на сервере секреты только читаются, хранить его негде.
