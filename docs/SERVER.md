...
В нём заполняется `chat_id` для Telegram; остальное уже под сервер: интерфейс без браузера, кластером управляет systemd, торговля выключена.
...
3. Вписать `chat_id` в `config.toml`, перезапустить: `sudo systemctl restart ludik`.
